...
class PoseEngine():
    ...
    def __init__(self, model_path, mirror=False, device='tpu'):
        ...
        """
        if device not in ['tpu', 'gpu', 'cpu']:
            raise ValueError('Device must be tpu, gpu, or cpu')
        
        delegates = []
        # Always load the decoder delegate
        delegates.append(load_delegate(POSENET_SHARED_LIB))
        
        if device == 'tpu':
            delegates.append(load_delegate(EDGETPU_SHARED_LIB))
        elif device == 'gpu':
             # Try standard TFLite GPU delegate, fallback might be needed depending on system
             try:
                 delegates.append(load_delegate('libOpenCL.so')) 
             except Exception as e:
                 print(f"Warning: Could not load GPU delegate: {e}")
